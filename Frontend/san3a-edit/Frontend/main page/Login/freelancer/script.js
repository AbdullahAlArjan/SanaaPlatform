document.addEventListener('DOMContentLoaded', () => {
    ...
    //    // Populate CV and Work Samples (if applicable)
    //    if (freelancerData) {
    //        const cvLink = document.createElement('a');
    //        cvLink.href = freelancerData.cv;
    //        cvLink.textContent = 'Download CV';
    //        cvLink.target = '_blank';
    //        document.querySelector('.profile-info').appendChild(cvLink);
   
    //        const workSamplesLink = document.createElement('a');
    //        workSamplesLink.href = freelancerData.workSamples;
    //        workSamplesLink.textContent = 'View Work Samples';
    //        workSamplesLink.target = '_blank';
    //        document.querySelector('.profile-info').appendChild(workSamplesLink);
    //    }
    ...
    const resetFreelancerPage = () => {
        // Reset the appState object
        appState = {
        ...
        // Clear the "My Services" section
        const servicesList = document.getElementById('services-list');
        if (servicesList) {
            servicesList.innerHTML = ''; // Remove all services
        }
    
        // Set the rating to 0/5
        const ratingElement = document.querySelector('.stats .stat-item:nth-child(1) h4');
        if (ratingElement) {
            ratingElement.textContent = '0/5';
        }
    
        // Set the number of clients to 0
        const clientsElement = document.querySelector('.stats .stat-item:nth-child(2) h4');
        if (clientsElement) {
            clientsElement.textContent = '0';
        }
    
        // Empty the client reviews section
        const testimonialCarousel = document.querySelector('.testimonial-carousel');
        if (testimonialCarousel) {
            testimonialCarousel.innerHTML = ''; // Remove all testimonials
        }
        ...
        // Clear the order list in the manage services modal
        const orderListContainer = document.getElementById('order-list-container');
        if (orderListContainer) {
            orderListContainer.innerHTML = ''; // Remove all orders
        }
    
        // Clear the manage services section
        const manageServicesContainer = document.getElementById('manage-services-container');
        if (manageServicesContainer) {
            manageServicesContainer.innerHTML = ''; // Remove all services
        }
    
        // Clear the notifications section
        const notificationList = document.getElementById('notificationList');
        if (notificationList) {
            notificationList.innerHTML = ''; // Remove all notifications
        }
    
        // Reset the notification count
        const notificationCount = document.getElementById('notificationCount');
        if (notificationCount) {
            notificationCount.textContent = '0'; // Set notification count to 0
            notificationCount.style.display = 'none'; // Hide the notification count
        }
    };

    // Call the reset function after registration
    resetFreelancerPage();
