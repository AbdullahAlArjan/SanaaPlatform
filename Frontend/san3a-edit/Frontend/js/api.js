...
function clearAuthAndRedirect() {
  ...
  // Build a path back to Login.html that works from any folder depth.
  const depth     = window.location.pathname.split('/').filter(Boolean).length - 1;
  const prefix    = depth > 0 ? '../'.repeat(depth) : '';
  const loginPath = prefix + 'Login/Login.html';

  // Case-insensitive check so /Login/Login.html doesn't trigger a redirect loop.
  if (!window.location.pathname.toLowerCase().endsWith('login.html')) {
    console.warn('[api] Session expired. Redirecting to:', loginPath);
