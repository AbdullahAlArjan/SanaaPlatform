...
namespace Sanaa.API.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        // GET /api/admin/freelancers  (all — no status filter, admin only)
        [HttpGet("freelancers")]
        public async Task<IActionResult> GetAllFreelancers()
        {
            var freelancers = await _freelancerService.GetAllFreelancerProfilesAsync();
        ...
        public async Task<IActionResult> ApproveFreelancer(int id)
        {
            ...
            if (!success)
                return NotFound(new { message = "الصنايعي غير موجود أو ليس لديه ملف مهني.", code = "NOT_FOUND" });

            return Ok(new { message = "تمت الموافقة على الصنايعي بنجاح.", code = "APPROVED" });
        }
        ...
        public async Task<IActionResult> RejectFreelancer(int id)
        {
            ...
            return Ok(new { message = "تم رفض طلب الصنايعي.", code = "REJECTED" });
        }
    }
}
